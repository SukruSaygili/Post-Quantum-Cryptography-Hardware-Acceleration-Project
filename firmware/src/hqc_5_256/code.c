...
void PQCLEAN_HQC256_CLEAN_code_decode(uint8_t *m, const uint64_t *em) {
    ...
    PQCLEAN_HQC256_CLEAN_reed_muller_decode(tmp, em); //endCycle = read_cycles(); uart_puts("\nRMdecode: "); uart_putint(endCycle - startCycle); startCycle = read_cycles();
    PQCLEAN_HQC256_CLEAN_reed_solomon_decode(m, tmp); //endCycle = read_cycles(); uart_puts("\nRSdecode: "); uart_putint(endCycle - startCycle);
}
