...
void PQCLEAN_HQC256_CLEAN_hqc_pke_keygen(uint8_t *pk, uint8_t *sk) {
    ...
    // Compute public key
    PQCLEAN_HQC256_CLEAN_vect_set_random(&pk_seedexpander, h); //startCycle = read_cycles();
    PQCLEAN_HQC256_CLEAN_vect_mul(s, y, h); //endCycle = read_cycles(); uart_puts("\nKeygen vect_mul: "); uart_putint(endCycle - startCycle); startCycle = read_cycles();
...
void PQCLEAN_HQC256_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, uint8_t *theta, const uint8_t *pk) {
    ...
    //startCycle = read_cycles();
    PQCLEAN_HQC256_CLEAN_vect_mul(tmp2, r2, s); //endCycle = read_cycles(); uart_puts("\nEnc vect_mul2: "); uart_putint(endCycle - startCycle); startCycle = read_cycles();
...
uint8_t PQCLEAN_HQC256_CLEAN_hqc_pke_decrypt(uint8_t *m, uint8_t *sigma, const uint64_t *u, const uint64_t *v, const uint8_t *sk) {
    ...
    // Compute v - u.y
    PQCLEAN_HQC256_CLEAN_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2); //startCycle = read_cycles();
    PQCLEAN_HQC256_CLEAN_vect_mul(tmp2, y, u); //endCycle = read_cycles(); uart_puts("\nDec vect_mul: "); uart_putint(endCycle - startCycle); startCycle = read_cycles();
    PQCLEAN_HQC256_CLEAN_vect_add(tmp2, tmp1, tmp2, VEC_N_SIZE_64);
